...
function Stat({ icon: Icon, tint, value, label, href, action }) {
  return (
    <Card className="p-4">
      ...
      <Link href={href} className="mt-1 inline-block text-xs font-medium text-urgent-600 hover:underline">
        {action}
      </Link>
    </Card>
  );
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8">
      <PageHeader
        ...
        action={
          <Button asChild variant="navy">
            <Link href="/publier">
              <Plus className="h-4 w-4 text-urgent-400" aria-hidden /> Publier un espace
            </Link>
          </Button>
        }
      />
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Stat icon={FileText} tint="text-urgent-500" value={active} label="Annonces actives"
          href="/mes-annonces" action="Voir mes annonces" />
        <Stat icon={Inbox} tint="text-blue-600" value={newRequests} label="Nouvelles demandes"
          href="/demandes" action="Voir les demandes" />
        <Stat icon={AlertTriangle} tint="text-red-500" value={expiring} label="Relâches dans 14 jours"
          href="/mes-annonces?filtre=urgent" action="Voir les détails" />
        <Stat icon={TrendingUp} tint="text-emerald-600" value={views} label="Vues cette semaine"
          href="/mes-annonces" action="Voir le détail" />
      </div>

      <Card className="mt-6 p-5">
        <h2 className="font-semibold text-navy-900">Que cherchez-vous aujourd'hui ?</h2>
        <form action="/marketplace" className="mt-3 flex gap-2">
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        <Link href="/marketplace" className="text-sm font-medium text-urgent-600 hover:underline">
          Voir tout
        </Link>
      </div>
      ...
      {latest.length === 0 ? (
        <Card className="mt-4 p-8 text-center">
          <p className="text-sm text-navy-500">
            Aucune annonce publiée par une autre agence pour l'instant. Publiez la vôtre pour
            amorcer le réseau.
          </p>
        </Card>
      ) : (
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {latest.map((l) => (
            <ListingCard key={l.id} listing={l} canSeePrice={canSeePrice(l, user)} compact />
          ))}
        </div>
      )}
    </div>
