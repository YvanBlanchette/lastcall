...
import { ChevronLeft, MapPin, Calendar, Ship, Globe, Bed, Users, Clock, CheckCircle2, UserRound } from "lucide-react";
import { prisma } from "@/lib/prisma";
import { requireUser, visibilityScopeFor, canSeePrice } from "@/lib/auth";
import { listingOwnerProfile, ownsListing } from "@/lib/org";
...
export default async function ListingPage({ params, searchParams }) {
	...
	const isOwner = ownsListing(listing, user);
	...
	const owner = listingOwnerProfile(listing);
	const agencyProfileHref = owner?.href ?? "#";
	...
	const facts = [
		...
		[
			...
			authorProfileHref ? (
				...
				`${listing.author?.firstName || ""} ${listing.author?.lastName || ""}`.trim() || "Non renseigné"
			),
		],
	];
	...
	return (
		<div className="page-shell page-shell-lg">
			<Link
				href="/marketplace"
				className="mb-4 inline-flex items-center gap-1 text-sm font-medium text-navy-600 hover:text-navy-900"
			>
				<ChevronLeft
					className="h-4 w-4"
					aria-hidden
			...
			{searchParams?.publiee && (
				<div
					role="status"
					className="mb-4 flex items-center gap-2 rounded-lg bg-emerald-50 p-3 text-sm font-medium text-emerald-800"
				>
					<CheckCircle2
						className="h-4 w-4"
						aria-hidden
				...
				<aside className="space-y-4">
					<Card className="p-5">
						{showPrice ? (
							...
							<p className="text-sm font-medium text-navy-500">Le tarif de ce groupe est réservé aux agences vérifiées.</p>
						)}

						<div className={cn("mt-4 flex items-start gap-2 rounded-lg p-3", days <= 21 ? "bg-red-50" : "bg-urgent-50")}>
							<Clock
								className={cn("mt-0.5 h-4 w-4 shrink-0", days <= 21 ? "text-red-600" : "text-urgent-600")}
								aria-hidden
							/>
							<div className="text-sm">
								<p className={cn("font-semibold", days <= 21 ? "text-red-700" : "text-urgent-700")}>Relâche le {formatDate(listing.releaseDate)}</p>
								<p className="text-navy-600">{days > 0 ? `Dans ${days} jour${days > 1 ? "s" : ""}` : "Aujourd'hui"}</p>
							</div>
						</div>
						...
						<div className="mt-4">
							<InterestDialog
								...
								agencyPublicIdentifier={owner?.kind === "AGENCY" ? listing.agency?.publicIdentifier : listing.ownerSupplier?.publicIdentifier}
								agencyId={owner?.id ?? ""}
								disabled={isOwner || user.status !== "VERIFIED" || Boolean(user.supplierId)}
								disabledReason={
									isOwner
										? "Cette annonce appartient à votre organisation."
										: user.supplierId
											? "Les comptes fournisseurs ne peuvent pas envoyer de demandes."
					...
					<Card className="p-5">
						<p className="text-xs uppercase tracking-wide text-navy-400">{owner?.kind === "SUPPLIER" ? "Fournisseur" : "Agence détentrice"}</p>
					...
					<p className="px-1 text-xs leading-relaxed text-navy-400">
						LastCall ne prend pas part à la réservation ni à la commission. Les conditions de collaboration se règlent entre les deux agences, selon les règles
						du fournisseur.
					</p>
				</aside>
			</div>
