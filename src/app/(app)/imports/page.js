...
const STATUS_LABELS = {
  UPLOADED: "Reçu", PARSING: "Lecture en cours", NEEDS_REVIEW: "À vérifier",
...
export default async function ImportsPage({ searchParams }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-8">
      ...
      {pending && pending.agencyId === user.agencyId ? (
        <div className="mt-6">
          <ImportReview batch={pending} />
        </div>
      ) : (
        <Card className="mt-6 p-6">
          <h2 className="font-semibold text-navy-900">Importez plusieurs groupes</h2>
          <p className="mt-1 text-sm text-navy-500">
            Utilisez le modèle LastCall : les colonnes y sont déjà nommées correctement.
          </p>
          <div className="mt-4">
            <ImportUploader />
          </div>
          ...
          >
            <Download className="h-4 w-4" aria-hidden /> Télécharger le modèle Excel LastCall
          </Link>
        </Card>
      )}

      <Card className="mt-6">
        <CardHeader><CardTitle>Vos imports récents</CardTitle></CardHeader>
        {batches.length === 0 ? (
          <p className="p-5 text-sm text-navy-500">
            Aucun import pour l'instant. Vos fichiers apparaîtront ici avec leur résultat.
          </p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wide text-navy-400">
                  <th scope="col" className="px-5 py-2 font-medium">Fichier</th>
                  <th scope="col" className="px-5 py-2 font-medium">Date</th>
                  <th scope="col" className="px-5 py-2 font-medium">Statut</th>
                  <th scope="col" className="px-5 py-2 font-medium">Résultat</th>
                </tr>
              </thead>
              <tbody>
                {batches.map((b) => (
                  <tr key={b.id} className="border-t border-navy-100">
                    <td className="px-5 py-3 font-medium text-navy-900">
                      {b.status === "NEEDS_REVIEW" ? (
                        <Link href={`/imports?lot=${b.id}`} className="hover:underline">{b.fileName}</Link>
                      ) : (
                        b.fileName
                      )}
                    </td>
      ...
      <Card className="mt-6 p-5">
        <div className="flex gap-3">
          <FileSpreadsheet className="h-5 w-5 shrink-0 text-navy-400" aria-hidden />
          <div className="text-sm text-navy-600">
            <p className="font-medium text-navy-900">Réimporter le même fichier met à jour vos annonces</p>
            <p className="mt-1 leading-relaxed">
              Si la colonne <code className="rounded bg-navy-100 px-1">external_id</code> est remplie,
              LastCall reconnaît le groupe et met à jour l'inventaire, le prix et la date de
              relâche au lieu de créer un doublon.
            </p>
          </div>
        </div>
      </Card>
