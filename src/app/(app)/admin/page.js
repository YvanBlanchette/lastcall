...
export default async function AdminPage() {
	...
	const [pendingAgencies, pendingSuppliers, activeListings, unmetSearches, repeatPublishers, bookedCount] = await Promise.all([
		prisma.agency.findMany({ where: { status: "PENDING" }, orderBy: { createdAt: "asc" } }),
		prisma.supplier.findMany({ where: { status: "PENDING", members: { some: {} } }, orderBy: { createdAt: "asc" } }),
	...
	return (
		<div className="page-shell page-shell-lg">
			<PageHeader
				...
				description="Vérification des agences et santé du réseau."
			/>
			...
			<div className="mt-6 grid gap-4 sm:grid-cols-3">
				<Card className="p-4">
					...
					<p className="mt-1 text-xs leading-relaxed text-navy-400">
						Au moins deux publications espacées de sept jours. C&apos;est le seul chiffre qui indique si l&apos;habitude est en train de naître.
					</p>
				</Card>
				<Card className="p-4">
			...
			<Card className="mt-6 p-5">
				<h2 className="section-title">Agences en attente de vérification</h2>
				{pendingAgencies.length === 0 ? (
					...
					<ul className="mt-3 divide-y divide-navy-100">
						{pendingAgencies.map((a) => (
							<AgencyVerifyRow
								key={a.id}
								agency={{ ...a, createdAtLabel: formatDate(a.createdAt) }}
							/>
						))}
					</ul>
				)}
			...
			<Card className="mt-6 p-5">
				<h2 className="section-title">Fournisseurs en attente de vérification</h2>
					...
					<ul className="mt-3 divide-y divide-navy-100">
						{pendingSuppliers.map((s) => (
							<AgencyVerifyRow
								key={s.id}
								kind="SUPPLIER"
								agency={{ ...s, createdAtLabel: formatDate(s.createdAt) }}
							/>
						))}
					</ul>
				)}
			...
			<Card className="mt-6 p-5">
				<h2 className="section-title">Demande non satisfaite (7 derniers jours)</h2>
				<p className="section-subtitle">Recherches sans aucun résultat. C&apos;est ce que le réseau cherche et que personne ne publie.</p>
