...
export async function commitImportAction(batchId) {
  ...
  if (!batch || batch.agencyId !== user.agencyId) {
    return { error: "Ce lot d'import n'appartient pas à votre agence." };
  }
  ...
  for (const row of publishable) {
    ...
    const listing = p.externalId
      ? await prisma.listing.upsert({
          where: { agencyId_externalId: { agencyId: user.agencyId, externalId: p.externalId } },
          update: {
...
export async function fixImportRowAction(rowId, patch) {
  ...
  if (!row || row.batch.agencyId !== user.agencyId) return { error: "Action non autorisée." };
