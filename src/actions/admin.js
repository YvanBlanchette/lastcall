...
export async function verifySupplierAction(supplierId, status) {
	const admin = await requireAdmin();
	if (!["VERIFIED", "REJECTED", "SUSPENDED"].includes(status)) {
		return { error: "Statut non valide." };
	}
	...
	await prisma.user.updateMany({
		where: { supplierMemberships: { some: { supplierId } } },
		data: { status },
	});

	await prisma.auditLog.create({
		data: { userId: admin.id, action: `SUPPLIER_${status}`, entityType: "Supplier", entityId: supplierId },
	});

	if (status === "VERIFIED") {
		await Promise.allSettled(supplier.members.map((m) => mail.accountVerified(m.user)));
	}
