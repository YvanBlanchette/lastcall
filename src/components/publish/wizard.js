...
import { createListingAction, updateListingAction, uploadListingImageAction } from "@/actions/listings";
...
function toInitialValues(initialListing) {
	if (!initialListing) return EMPTY;
	return {
		travelType: initialListing.travelType ?? "",
		supplierId: initialListing.supplierId ?? "",
		supplierName: "",
		title: initialListing.title ?? "",
		destination: initialListing.destination ?? "",
		shipName: initialListing.ship?.name ?? "",
		departureCity: initialListing.departureCity ?? "",
		departureDate: toDateInput(initialListing.departureDate),
		returnDate: toDateInput(initialListing.returnDate),
		language: initialListing.language ?? "fr",
		inventoryType: initialListing.inventoryType ?? "CABINS",
		inventoryLeft: String(initialListing.inventoryLeft ?? ""),
		inventoryTotal: initialListing.inventoryTotal ? String(initialListing.inventoryTotal) : "",
		cabinCategory: initialListing.cabinCategory ?? "",
		price: initialListing.price != null ? String(initialListing.price) : "",
		currency: initialListing.currency ?? "CAD",
		releaseDate: toDateInput(initialListing.releaseDate),
		groupBenefits: initialListing.groupBenefits ?? "",
		conditions: initialListing.conditions ?? "",
		commissionSplit: initialListing.commissionSplit ?? "Ouvert à discuter",
		notes: initialListing.notes ?? "",
		visibility: initialListing.visibility ?? "B2B_ONLY",
		soloAvailable: Boolean(initialListing.soloAvailable),
		guaranteed: Boolean(initialListing.guaranteed),
		priceHidden: Boolean(initialListing.priceHidden),
		externalId: initialListing.externalId ?? "",
	};
}

function toInitialImages(initialListing) {
	if (!initialListing?.images?.length) return [];
	return initialListing.images.map((img) => ({
...
export function PublishWizard({ suppliers, cities, initialListing = null, lockedSupplier = null }) {
	...
	const [images, setImages] = useState(() => toInitialImages(initialListing));
	const [uploading, setUploading] = useState(false);
	const [uploadError, setUploadError] = useState(null);
	const [state, formAction] = useFormState(isEditing ? updateListingAction : createListingAction, {});
	...
	return (
		<form action={formAction}>
          ...
          précédentes : l'utilisateur peut revenir en arrière sans rien perdre. */}
					...
					>
						<span
							...
							className={cn(
								...
								i < step ? "bg-emerald-500 text-white" : i === step ? "bg-urgent-500 text-white" : "bg-navy-100 text-navy-400",
							)}
						>
							{i < step ? (
								<Check
						...
						<span className={cn("hidden truncate text-xs sm:block", i === step ? "font-semibold text-navy-900" : "text-navy-400")}>{s}</span>
						{i < STEPS.length - 1 && (
							<span
								className="h-px flex-1 bg-navy-100"
								aria-hidden
							/>
						)}
					</li>
			...
			<div className="mt-6 rounded-xl bg-white p-6 shadow-sm ring-1 ring-navy-100">
						...
						<div className="mt-5 space-y-3">
							{TYPES.map((t) => (
								<button
									...
									type="button"
									onClick={() => {
										set("travelType", t.value);
										set("inventoryType", t.value === "CRUISE" ? "CABINS" : "SEATS");
									}}
								...
								>
									<t.icon
										className={cn("h-5 w-5", d.travelType === t.value ? "text-urgent-500" : "text-navy-400")}
										aria-hidden
									/>
									<span>
						...
						<div className="mt-5 space-y-4">
							...
							>
								<Input
									...
									placeholder="Ex. Découverte du Japon au printemps"
							...
							<div className="grid gap-4 sm:grid-cols-2">
								...
								>
									<Input
										...
										placeholder="Ex. Norwegian Encore"
									/>
								</Field>
								<Field
						...
						<div className="mt-5 space-y-4">
							<Field label="Type d'inventaire">
								<div className="grid grid-cols-2 gap-3">
									{[
										{ v: "CABINS", l: "Cabines", i: Bed },
										{ v: "SEATS", l: "Places", i: Users },
									].map((o) => (
										<button
											key={o.v}
											type="button"
											...
											className={cn(
												...
												d.inventoryType === o.v ? "border-urgent-500 bg-urgent-50 text-urgent-700" : "border-navy-200 text-navy-600 hover:border-navy-400",
											)}
										>
											<o.i
												className="h-4 w-4"
							...
							<div className="grid gap-4 sm:grid-cols-2">
								...
								>
									<Input
										...
										type="number"
										min="1"
										value={d.inventoryLeft}
										onChange={(e) => set("inventoryLeft", e.target.value)}
										placeholder="6"
									/>
								</Field>
								<Field
									label="Inventaire total du groupe"
									htmlFor="total"
									hint="Facultatif — sert au taux de remplissage."
								>
									<Input
										id="total"
										type="number"
										min="1"
										value={d.inventoryTotal}
										onChange={(e) => set("inventoryTotal", e.target.value)}
										placeholder="30"
									/>
								</Field>
							</div>
							...
							>
								<Input
									id="cat"
									value={d.cabinCategory}
									onChange={(e) => set("cabinCategory", e.target.value)}
				...
				{step === 3 && (
					<>
						...
						<div className="mt-5 space-y-4">
							<Field
								label="Avantages de groupe"
								htmlFor="avantages"
							>
								<Textarea
									id="avantages"
									rows={2}
									value={d.groupBenefits}
							...
							<Field
								label="Partage de commission"
								htmlFor="commission"
								hint="Ce que vous proposez à l'agence qui amène le client. LastCall ne prélève rien."
							>
								<Select
									id="commission"
									value={d.commissionSplit}
									onChange={(e) => set("commissionSplit", e.target.value)}
								>
					...
					<>
						...
						<div className="mt-5 overflow-hidden rounded-xl ring-1 ring-navy-100">
								...
								{releaseDays !== null && releaseDays <= 21 && (
									<>
										<div className="absolute left-3 top-3">
											<Badge tone="urgent">Release bientôt</Badge>
										</div>
										<div className="absolute right-3 top-3 rounded-lg bg-white px-2.5 py-1.5 text-center shadow-sm">
											...
											<div className="text-[9px] uppercase leading-tight text-navy-400">
												jours
												<br />
												restants
											</div>
							...
							<div className="p-4">
								...
								<div className="mt-3 flex items-end justify-between">
									...
									<span className="text-base font-bold text-navy-900">{d.price ? formatMoney(d.price, d.currency) : "—"}</span>
								</div>
							</div>
						</div>
						...
						{state?.errors && (
							...
							>
								...
								<ul className="mt-1 list-inside list-disc">
									{Object.values(state.errors).map((m) => (
										<li key={m}>{m}</li>
									))}
								</ul>
							</div>
						...
						<div className="mt-5 space-y-2">
							<SubmitButton
								...
								value="publish"
								className="w-full"
								pendingLabel={isEditing ? "Mise à jour…" : "Publication…"}
							>
								{isEditing ? "Mettre à jour l'annonce" : "Publier sur LastCall"}
							</SubmitButton>
							<SubmitButton
								name="intent"
								value="draft"
							...
							>
								{isEditing ? "Enregistrer les changements en brouillon" : "Enregistrer en brouillon"}
							</SubmitButton>
						</div>
					</>
				)}
			...
			<p className="mt-3 text-center text-xs text-navy-400">Vos réponses sont conservées si vous revenez en arrière.</p>
		</form>
	);
}
