...
export function InterestDialog({ listingId, agencyName, agencyPublicIdentifier, agencyId, disabled, disabledReason }) {
	const [open, setOpen] = useState(false);
	const [state, formAction] = useFormState(createInterestRequestAction, {});
	const toast = useToast();
	const agencyProfileHref = agencyPublicIdentifier ? `/@${agencyPublicIdentifier}` : agencyId ? `/profil-public/${agencyId}` : null;
	...
	return (
		<>
			...
			<p className="mt-2 text-center text-xs text-navy-400">
				...
				. Aucune coordonnée n&apos;est partagée avant son accord.
			</p>
				...
				>
					<div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
						...
						<p className="mt-1 text-sm text-navy-500">
							...
							reçoit votre demande et décide de la suite. Vos clients restent les vôtres.
						</p>
						...
						>
							...
							<Field
								...
								htmlFor="pax"
								error={state?.errors?.numberOfTravelers}
								required
							>
								<Input
									id="pax"
