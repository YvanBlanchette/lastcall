...
import { Eye, Inbox, Minus, Plus } from "lucide-react";
...
const STATUS_TONE = {
  ...
  SOLD_OUT: "neutral", RELEASED: "neutral", EXPIRED: "neutral", ARCHIVED: "neutral",
};
...
export function ListingRow({ listing }) {
  ...
  return (
    <Card className="flex flex-wrap items-center gap-4 p-4">
      <Link href={`/listing/${listing.id}`} className="flex min-w-0 flex-1 items-center gap-4">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 flex items-center gap-3 text-xs text-navy-400">
            <span className="flex items-center gap-1"><Eye className="h-3 w-3" aria-hidden /> {listing._count?.views ?? 0}</span>
            <span className="flex items-center gap-1"><Inbox className="h-3 w-3" aria-hidden /> {listing._count?.requests ?? 0}</span>
          </p>
        </div>
      </Link>

      <div className="flex items-center gap-1" aria-label="Ajuster l'inventaire restant">
        <button
          onClick={() => change(-1)}
        ...
        <span className="w-12 text-center text-sm font-semibold text-navy-900">
          {left}
        </span>
        <button
          onClick={() => change(1)}
      ...
      <div className="text-right">
        <p className={cn("text-xs", days <= 14 ? "font-semibold text-red-600" : "text-navy-500")}>
          {days > 0 ? `Relâche dans ${days} j` : "Relâchée"}
        </p>
        <button onClick={pause} disabled={pending} className="text-xs text-navy-400 hover:text-navy-900 hover:underline">
