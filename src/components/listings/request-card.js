...
export function RequestCard({ request, role }) {
	...
	const buyerAgencyHref = request.buyerAgency
		? request.buyerAgency.publicIdentifier
			? `/@${request.buyerAgency.publicIdentifier}`
			: `/profil-public/${request.buyerAgency.id}`
		: null;
	const listingOwner = request.listing?.ownerSupplier ?? request.listing?.agency ?? null;
	const listingAgencyHref = listingOwner ? (listingOwner.publicIdentifier ? `/@${listingOwner.publicIdentifier}` : `/profil-public/${listingOwner.id}`) : null;
	...
	return (
		<Card className="p-5">
			<div className="flex flex-wrap items-start justify-between gap-2">
				<div className="min-w-0">
					...
					<p className="text-xs text-navy-500">
						{role === "seller" ? (
							<>
								{request.buyer?.publicIdentifier ? (
									<Link
										href={`/@${request.buyer.publicIdentifier}`}
								...
								{buyerAgencyHref ? (
									<Link
										href={buyerAgencyHref}
										className="font-medium text-urgent-600 hover:underline"
									>
										{request.buyerAgency.name}
									</Link>
								) : (
									request.buyerAgency.name
								)}
			...
			{request.message && <p className="mt-3 rounded-lg bg-navy-50 p-3 text-sm text-navy-600">{request.message}</p>}
			...
			{request.status === "CONNECTED" && request.conversation && (
				<div className="mt-3">
					<Link
						href={`/messagerie?c=${request.conversation.id}`}
			...
			{role === "seller" && request.status === "CONNECTED" && !request.outcomeBooked && (askOutcome || true) && (
				<div className="mt-3 rounded-lg border border-dashed border-navy-200 p-3">
					...
					<p className="mt-0.5 text-xs text-navy-500">
						LastCall ne voit pas vos transactions. Votre réponse est la seule mesure de ce que le réseau fait vraiment vendre.
					</p>
			...
			{request.outcomeBooked && (
				<p className="mt-3 flex items-center gap-1.5 text-sm font-medium text-emerald-700">
					<Check
						...
						aria-hidden
					/>{" "}
					Réservation confirmée par l&apos;agence détentrice
				</p>
			)}
		</Card>
	);
